...
1. If he named tracks that don't exist yet, either **create them in the app
   first** (a completion must match an existing track, case-insensitively, or
   the row stays in review) or pass `--create-missing-tracks` to have the
   import create them as name-only stub tracks (reported in the run output;
   Charlie enriches them in-app). Decided 2026-07-20: stubs are OK, no tags.
...
**Bulk-invoice splits** (since 2026-07-20): a `row_key` may appear on several
completion rows — one per track, each carrying its share of the invoice in
`amount`. Each row becomes its own License + Invoice; the group must sum
exactly to the source row's amount or the run aborts before writing anything.
The completions CSV therefore needs `row_key`, `track_name` **and `amount`**
columns. Duplicate (row_key, track) rows abort.

**The "?" track**: rows whose track isn't in Charlie's library at all complete
to the literal track name `?` (a placeholder track; created by
`--create-missing-tracks` on first use). It is deliberately excluded from
auto-matching. Charlie reassigns those licenses in-app when he identifies the
real track — license track is editable in the app.

**Charlie's 2026-07 completions file** (`Fixed Ingestion Errors -
needs-review.csv`) is NOT run directly — his answers sit in an inserted
`TRACK NAME` column and four bulk groups had amount errors. The cleaned,
runnable file is `~/Downloads/license-completions-clean.csv`, generated by the
transform documented in the session that produced it (all 80 keys, 110 rows,
$258,018.19 total).
