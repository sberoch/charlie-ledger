...
// source — either one License or one Demo (never both, never neither). Bill-to,
// amount, and description are snapshotted at issue so it is a self-contained
// historical document. Status (Paid/Unpaid/Overdue/Voided) is derived, not
// stored. Never hard-deleted — cancelled invoices are voided. See ADR-0001.
export const invoice = pgTable(
  ...
  (table) => [
    // Nullable uniques enforce the 1:1: at most one invoice per License/Demo,
    // while the many uninvoiced sources stay null.
    uniqueIndex("invoice_license_idx").on(table.licenseId),
    uniqueIndex("invoice_demo_idx").on(table.demoId),
