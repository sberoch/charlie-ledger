...
export const license = pgTable(
  'license',
  {
    ...
    notes: text('notes'),
    // Optional freeform grant scope (cuts, use/territories, term wording) that
    // is snapshotted onto the invoice description at issue. Distinct from
    // `notes`, which stays private. Empty => an auto-composed line. See ADR-0003.
    terms: text('terms'),
